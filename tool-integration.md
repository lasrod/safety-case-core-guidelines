...
With each pass request, send this instruction verbatim, with `{question}` replaced by the pass's question: "This request is one pass of a review that is split into several passes. The question for this pass is: {question} Review the selected element only against the guidelines in this request, and do not report a finding under any other guideline; the other passes cover them. Where a note in this request says to cite a guideline that is not in this request for some defect, that defect is not a finding in this pass; the pass that carries that guideline reports it."
