...
10. **Deliver the neighbour notes with the guidelines.** Where a guideline carries `distinguish_from`, send the notes with it. They say which of two confusable guidelines to cite for a defect, so that two tools, or two runs of one tool, cite the same id for the same finding. Send them in a pass request too, including notes that name a guideline of another pass: `review_pass_instruction` relies on them to leave that defect to the other pass.
...
Changes in content `0.10.0`, from `0.9.0`, which change behaviour without changing the contract's shape (issue #19):

- **RD.2 covers confidence language only.** Its title, statement, short rule, review prompts, and repair no longer ask for the operating scope; a claim whose scope is stated nowhere in the branch, neither in the claim nor in its context, is AR.6. The bad example is unchanged, its problem text no longer names the missing scope, and the good example now keeps scope and assumptions in their own elements, as AR.7 asks. A check calibrated against the good example should be re-run. The check id `check-scope-and-confidence-language` is kept, because check ids are stable, but it now examines confidence language only, and a tool should stop reporting a missing scope under RD.2. RD.2 and AR.6 now carry `distinguish_from` notes naming each other.
- **Neighbour notes across passes.** `review_pass_instruction` gained a sentence: where a note in a pass request says to cite a guideline that is not in that request, the defect is not a finding in that pass. Another pass reports it if the profile applies that guideline; otherwise it is outside the profile, as it is in a single-request review of that profile. In `claim_review` the one such note is LF.5's, which names EV.5, applied by `evidence_review` and `justification_review`. Without the sentence, a pass reported such a defect under its own nearest guideline while the other pass reported it under the right one. A tool that sends the instruction verbatim needs no change; a tool that has re-implemented it should add the rule, and every tool fanning out over passes must deliver `distinguish_from` notes with each pass, as item 10 above says. `review_pass_merge` is unchanged.
- **Wording pass question.** The `claim_review` wording pass no longer asks whether the claim is scoped, which no guideline in that pass now covers; it asks whether the claim is no more confident than its support.
...
With each pass request, send this instruction verbatim, with `{question}` replaced by the pass's question: "This request is one pass of a review that is split into several passes. The question for this pass is: {question} Review the selected element only against the guidelines in this request, and do not report a finding under any other guideline; the other passes cover them. Where a note in this request says to cite a guideline that is not in this request for some defect, that defect is not a finding in this pass: another pass reports it if this review applies that guideline, and otherwise it is outside this review."
...
| --- | --- | --- |
| Wording (`wording`) | Is the claim stated well, as one falsifiable and unambiguous proposition, bounded, plainly worded, and no more confident than its support? | CL.1, CL.2, CL.3, CL.4, CL.5, CL.6, RD.2, RD.4, RD.5 |
