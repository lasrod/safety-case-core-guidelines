...
Two registry-level sentences make a fanned-out review the same across tools. `review_pass_instruction` is sent verbatim with each pass request, with `{question}` replaced by the pass's question. It also says that where a neighbour note assigns a defect to a guideline that is not in the pass, the defect is not a finding in that pass: another pass reports it if the profile applies that guideline, and otherwise it is outside the profile, as it is in a single-request review. One defect is then not reported once per pass under two ids. `review_pass_merge` says that a finding counts only for the pass that carries its guideline, so one cited under another pass's guideline is discarded, and that a review with a pass that did not complete is incomplete rather than clean.
